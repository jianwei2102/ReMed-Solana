    ...
    if (connection && wallet) {
      fetchAuthPatient(connection, wallet).then((response) => {
        if (response.status === "success") {
          setAuthorized((response.data as { authorized: AuthorizedPatient[] })?.authorized.reverse())
        }
      });
    }
  }, [connection, wallet]);
  ...
  return <div>
    ...
    <div className="flex flex-row justify-between rounded-3xl text-white text-xl bg-[#37CAEC] mb-8">
      <div className="flex flex-col justify-center items-start pl-10 gap-3">
        ...
        <div>
          Welcome to <span className="font-semibold">ReMed</span>,
          where you deliver trusted healthcare and manage patient records with ease!
        </div>
      </div>
